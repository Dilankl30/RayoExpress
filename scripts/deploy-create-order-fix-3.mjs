...
BEGIN
  ...
  -- 11. Insert order items using direct select insert
  for v_i in 1..v_product_count loop
    v_product_id := p_product_ids[v_i];
    v_quantity := p_quantities[v_i];

    insert into public.order_items (order_id, product_id, product_name, quantity, unit_price)
    select v_order_id, id, name, v_quantity, price
    from public.products
...
try {
  ...
  console.log('Successfully deployed corrected create_order function with atomic order_items insert!');
} catch (err) {
