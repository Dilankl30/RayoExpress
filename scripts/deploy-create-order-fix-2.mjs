...
BEGIN
  ...
  -- FIX: Use id instead of user_id since id is the primary key and references profiles.id/users.id in customers table
  select id into v_customer_record from public.customers where id = v_customer_id;
  if not found then
...
try {
  ...
  console.log('Successfully deployed corrected create_order function using customers.id!');
} catch (err) {
