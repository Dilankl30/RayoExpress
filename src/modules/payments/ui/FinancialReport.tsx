    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        {(['week', 'month', 'all'] as const).map((p) => (
          <button
            key={p}
            onClick={() => setPeriod(p)}
            className={`px-4 py-2 rounded-xl text-xs font-medium ${period === p ? 'text-white shadow-md' : 'bg-gray-100 text-gray-600'}`}
            style={period === p ? { backgroundColor: '#6D28D9' } : {}}
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-2xl font-bold" style={{ color: '#22C55E' }}>${totalRevenue.toFixed(2)}</p>
          <p className="text-xs text-gray-400">Ingresos verificados</p>
        </div>
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-2xl font-bold text-gray-900">{totalOrders > 0 ? (totalRevenue / totalOrders).toFixed(2) : '0.00'}</p>
          <p className="text-xs text-gray-400">Promedio por pedido</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <p className="text-sm font-medium text-gray-900 mb-3">Historial de transacciones</p>
        {filtered.length === 0 ? (
          <p className="text-gray-400 text-sm text-center py-4">Sin transacciones en este período</p>
        ) : (
            ...
            {filtered.map((p) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm text-gray-900 capitalize">{p.method}</p>
                    <p className="text-xs text-gray-400">{new Date(p.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-gray-900">${p.amount.toFixed(2)}</p>
                  <span className={`text-xs ${p.verified ? 'text-green-600' : 'text-amber-600'}`}>
