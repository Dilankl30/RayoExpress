...
export const ALLOWED_TRANSITIONS: Record<OrderStatus, { to: OrderStatus[]; by: Role[] }> = {
  ...
  picked_up:  { to: ['on_the_way', 'cancelled'], by: ['driver', 'admin'] },
  on_the_way: { to: ['arrived', 'cancelled'], by: ['driver', 'admin'] },
  arrived:    { to: ['delivered', 'cancelled'], by: ['driver', 'admin', 'customer'] },
