      ...
      <div className="pt-10 pb-5 px-4" style={{ background: 'linear-gradient(160deg, #6D28D9, #4C1D95)' }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full cursor-pointer" style={{ backgroundColor: isOpen ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)' }} onClick={() => setIsOpen(!isOpen)}>
                ...
                {pendingOrders.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="bg-white rounded-2xl p-3 shadow-sm flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: stInfo.bg }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: stInfo.bg, color: stInfo.text }}>
                            {stInfo.label}
                          </span>
                        </div>
                      ...
                      {st !== 'ready' && st !== 'delivered' && (
                        <button onClick={() => advanceStatus(order.id)} className="px-3 py-1.5 rounded-xl text-xs text-white flex-shrink-0" style={{ backgroundColor: '#6D28D9' }}>
                          Avanzar
                        </button>
                      )}
                    </div>
        ...
        {activeTab === 'catalog' && (
          <div className="px-4 pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-white text-sm" style={{ backgroundColor: '#6D28D9' }}>
                <Plus size={14} />
                Nuevo
              </button>
            </div>
                ...
                <div className="space-y-2">
                  {[1, 2].map((j) => (
                    <div key={j} className="bg-white rounded-2xl p-3 shadow-sm flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#F3F4F6', fontSize: 24 }}>
                        🍔
                      </div>
                      <div className="flex-1">
