...
alter table public.orders add column if not exists delivery_lat float;
alter table public.orders add column if not exists delivery_lng float;

-- Drop previous versions to prevent signature overloading
