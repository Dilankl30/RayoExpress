...
export function OrderChat({ orderId, storeId, storeName, storeEmoji, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-card w-full sm:max-w-md sm:rounded-2xl sm:mx-4 max-h-[80vh] flex flex-col rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border-light" style={{ background: 'linear-gradient(160deg, var(--brand), var(--brand-dark))' }}>
          <div className="flex items-center gap-2">
            <span style={{ fontSize: 20 }}>{storeEmoji}</span>
            <p className="text-white font-medium text-sm">{storeName}</p>
          </div>
          <button onClick={() => onClose?.()} className="w-7 h-7 rounded-full bg-card/20 flex items-center justify-center">
            ...
            messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`max-w-[80%] px-3 py-2 rounded-2xl ${isMine ? 'text-white' : 'bg-card text-text-primary shadow-sm border border-border-light'}`}
        ...
        <div className="flex items-center gap-2 p-3 border-t border-border-light bg-card">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-surface-hover outline-none text-sm text-text-primary placeholder:text-text-secondary"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim()}
