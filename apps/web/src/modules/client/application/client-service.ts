...
async function resolveGeolocation(): Promise<{ coords: [number, number]; city: string | null } | null> {
  ...
  try {
    ...
    const city = await detectCityCached(coords[0], coords[1]);
    return { coords, city };
  } catch {
    return null;
  }
...
export async function resolvePreferredLocation(userId: string | undefined | null): Promise<ResolvedLocation> {
  if (!userId) {
