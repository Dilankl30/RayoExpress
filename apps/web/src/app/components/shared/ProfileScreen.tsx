...
import { getDriverHiringEnabled } from '../../../modules/app/application/app-config.service';
...
export function ProfileScreen() {
  ...
  const [driverHiringEnabled, setDriverHiringEnabled] = useState(true);

  useEffect(() => {
    if (!userId) return;

    setLegalAccepted(localStorage.getItem(`rayo-legal-accepted:${userId}`) === 'true');
    getDriverHiringEnabled()
      .then(setDriverHiringEnabled)
      .catch(() => setDriverHiringEnabled(true));
  }, [userId]);

  const acceptLegalTerms = () => {
    if (!userId) return;
    localStorage.setItem(`rayo-legal-accepted:${userId}`, 'true');
  ...
  const quickActions = [
    ...
    { label: 'Notificaciones', icon: Bell, screen: 'notification-settings' as Screen },
  ] as const;
    ...
    <div className="min-h-screen bg-surface pb-16 lg:pb-10">
      <div className="lg:hidden pt-10 pb-6 px-4" style={{ background: 'linear-gradient(160deg, var(--brand), var(--brand-dark))' }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-white/60 text-xs">{user.phone || 'Agrega tu telefono'}</p>
          </div>
        </div>
      </div>
      ...
      <main className="px-4 -mt-4 lg:mt-0 lg:px-6 lg:pt-8 max-w-5xl mx-auto">
        <div className="hidden lg:flex items-center justify-between gap-6 mb-6">
          <div>
            ...
            <p className="text-text-secondary mt-1">Administra tu informacion, direcciones, favoritos y preferencias.</p>
          </div>
        ...
        <section className="bg-card rounded-2xl p-5 shadow-sm">
          <h2 className="text-sm font-bold text-text-primary mb-3">Acceso rapido</h2>
        ...
        <section className="bg-card rounded-2xl mt-3 p-5 shadow-sm">
          ...
          <MenuRow icon={Ticket} label="Cupones y promociones" sub="Codigos y descuentos" onClick={() => navigate('promotions')} />
        </section>

        <section className="bg-card rounded-2xl mt-3 p-5 shadow-sm">
          <h2 className="text-sm font-bold text-text-primary mb-1">Configuracion</h2>
          ...
          <MenuRow
            ...
            sub="Terminos, privacidad y uso de datos"
            badge={legalAccepted ? 'Aceptado' : 'Pendiente'}
            onClick={() => setShowLegal(true)}
          />
          {driverHiringEnabled && <MenuRow icon={Bike} label="Trabajar con nosotros" onClick={() => navigate('register-driver')} />}
          <MenuRow icon={Store} label="Registrar mi negocio" sub="Unete como tienda" onClick={() => navigate('register-store')} />
      ...
      {showLegal && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center p-4">
          <div className="bg-card rounded-3xl w-full max-w-lg p-5 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide" style={{ color: 'var(--brand)' }}>RayoExpress</p>
                <h2 className="text-xl font-black text-text-primary mt-1">Terminos y privacidad</h2>
              </div>
              <button type="button" onClick={() => setShowLegal(false)} className="w-9 h-9 rounded-full bg-surface text-text-secondary">
                x
              </button>
            </div>
            ...
            <div className="mt-4 space-y-3 text-sm text-text-secondary leading-relaxed">
              <p>
                Al usar RayoExpress aceptas que procesemos tus datos de cuenta, contacto, direcciones, pedidos,
                ubicacion de entrega y preferencias para operar el servicio.
              </p>
              <p>
                Tus datos se usan para crear pedidos, coordinar entregas, enviar notificaciones, prevenir abuso
                y cumplir obligaciones legales. Puedes actualizar tus datos desde tu perfil.
              </p>
              <p>
                Las tiendas y repartidores solo reciben la informacion necesaria para preparar y entregar tu pedido.
              </p>
            </div>

            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button type="button" onClick={() => setShowLegal(false)} className="py-3 rounded-2xl bg-surface text-text-primary font-bold">
                Revisar despues
              </button>
              <button type="button" onClick={acceptLegalTerms} className="py-3 rounded-2xl text-white font-bold" style={{ backgroundColor: 'var(--brand)' }}>
                Acepto
              </button>
            </div>
          </div>
