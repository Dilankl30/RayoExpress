...
import {
  MapPin, Search, Star, X, Clock, Compass, Home, Locate, Plus, Minus, ShoppingCart
} from 'lucide-react';
...
export function ExploreScreen() {
    ...
    const loadCoverageAndStores = async () => {
      try {
        ...
        if (storeIds.length > 0) {
          try {
            const products = await getProductsByStores(storeIds);
            const map: Record<string, Set<string>> = {};
            for (const product of products) {
              if (!product.category_id || !product.store_id) continue;
              if (!map[product.category_id]) {
                map[product.category_id] = new Set();
              }
              map[product.category_id].add(product.store_id);
            }
            setCategoryStoreMap(map);
          } catch (productError) {
            console.error('Error loading category store map:', productError);
            setCategoryStoreMap({});
          }
        } else {
          setCategoryStoreMap({});
  ...
  useEffect(() => {
    const loadUserAddressAndCoords = async () => {
      if (!user) return;
      try {
        const location = await resolvePreferredLocation(user.id);
        if (location.address?.line1) setUserAddress(location.address.line1);
        if (location.coords) setUserCoords(location.coords);
        if (!location.address && !location.coords) {
          setUserAddress('Ubicación de entrega');
        }
      } catch {
        // Ignored
      }
  ...
  }, [stores, search, activeCategory, categoryStoreMap]);
